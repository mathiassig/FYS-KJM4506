...
if __name__ == "__main__":
    ...
    # no background for PIPS detector, vaccuum chamber
     ...
     # Alpha# 
    if calibration_plot_alpha:
        calibration = ManageData("Pu238.Spe") # use this to find calibration parameters
        # this calibration spectrum used 244Cm and 238Pu
        if fit_switch:
            calibration.plot_data(show_plot=False, label=r"238Pu data")
            width =5
            max = 433 # bin number of peak, since this is pre-calibrated spectrum # 5.457 MeV
            calibration.get_fit(
                curve_start = max-width,
                curve_stop = max+width,
                init_guess = [10000, max, width],   # Amplitude, mean, variance. Guess for normal distribution 
                plot_fit = True,
                show_plot = False, # are plotting more peaks
                model = "normal"
            ) 
            calibration2 = ManageData("Pu238.Spe") # use this to find calibration parameters
            calibration2.plot_data(show_plot=False, label=r"238Pu data")
            width = 5
            max = 440 # bin number of peak, since this is pre-calibrated spectrum # 5.499 MeV
            calibration2.get_fit(
                curve_start = max-width,
                curve_stop = max+width,
                init_guess = [30000, max, width],  # Amplitude, mean, variance, slope, intercept. Guess for normal distribution with slope.
                plot_fit = True,
                show_plot = False,
                model = "normal"
            )
            calibration3 = ManageData("Cm244.Spe") # use this to find calibration parameters
            calibration3.plot_data(show_plot=False, label=r"244Cm data")
            width = 5
            max = 479 # bin number of peak, since this is pre-calibrated spectrum # 1332 keV
            calibration3.get_fit(
                curve_start = max-width,
                curve_stop = max+width,
                init_guess = [10000, max, width],  # Amplitude, mean, variance, slope, intercept. Guess for normal distribution with slope.
                plot_fit = True,
                show_plot = False,
                model = "normal"
            )
            calibration4 = ManageData("Cm244.Spe") # use this to find calibration parameters
            calibration4.plot_data(show_plot=False, label=r"244Cm data")
            width = 5
            max = 487 # bin number of peak, since this is pre-calibrated spectrum # 1332 keV
            ...
            calibration.plot_data(show_plot=True, label=r"238Pu data")
            calibration4 = ManageData("Cm244.Spe") # use this to find calibration parameters
            calibration4.plot_data(show_plot=True, label=r"244Cm data")

    ####  Beta  ###
    if calibration_plot_beta:
