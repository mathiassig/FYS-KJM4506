...
if __name__ == "__main__":
    ...
    background = ManageData("background.Spe", create_fig=False)
    # calibration = ManageData("calibration.Spe", create_fig=False) # use this to find calibration parameters

    # # plot calibration spectrum
    # Cali = ManageData("137Cs.Spe")
    # Cali.calibrate_data(calibration=False, background=background.signal, scale=False)
    # Cali.plot_data(show_plot=True, label=r"$^Calibration data")
    ...
    adjust = -50
    Cs137.get_fit(
        curve_start = 450 - adjust+200,
        curve_stop = 630 + adjust+240,
        init_guess = [10000, Cs_gamma, 20, -1e-3, 0],   # Amplitude, mean, variance, slope, intercept. Guess for normal distribution with slope.
        plot_fit = True,
        show_plot = True,
        model = "normal"
